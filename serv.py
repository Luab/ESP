...
def init_db():
    with app.app_context():
        db = get_db()
        db.cursor().execute('CREATE TABLE IF NOT EXISTS data (tok REAL NOT NULL DEFAULT 0, time INTEGER DEFAULT 0, mA INTEGER);') #Sergay said 'ebosh v tselih', so it should be fine
        db.commit()
